<template>
  <div class="category-container" :class="pageClasses">
    ...
    <div class="category-wrap">
      <router-link class="category-item" :to="{ path: '/tag/' }">
        <span class="category-text">所有标签</span>
      </router-link>
      <router-link class="category-item" :to="{ path: '/tag/' + key }"  v-for="value, key in $tags" :key="key">
      ...
      <div class="archive-item" v-for="value, key in archives" :key="key">
        ...
        <ul>
          <li v-for="post in value">
            <router-link class="title" :to="{ path: post.path }">{{ post.frontmatter.title || post.frontmatter.description }}</router-link>
            <span class="meta-info">
              (
              <span class="time">
                <time >{{ (post.publishDate || post.lastUpdated) | toLocaleString }}</time>
              </span>
              ...
              <span class="tags" v-if="post.frontmatter.tags && post.frontmatter.tags.length > 0">
                |
                标签:
                <router-link class="category-item em" v-for="item in post.frontmatter.tags" :to="{ path: '/tag/' + item }">{{item}}</router-link>
...
import { archivePostsByTag, isArray } from '../util'
...
export default {
  ...
  computed: {
    ...
    archives () {
      return archivePostsByTag(this.$posts, this.tag)
    },
    pageClasses () {
