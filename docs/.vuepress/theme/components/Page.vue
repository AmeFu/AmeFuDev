<template>
  <main class="page">
    <slot name="top"/>
    <PostHeader :post="post" v-if="!$page.frontmatter.noPostHeader" />
    ...
    <Content v-if="contentShow" style="padding: 20px 0;" />
    <Copyright v-if="!$page.frontmatter.noCopyright && false" />
    <footer class="page-edit" v-if="!$page.frontmatter.noPageEdit">
    ...
    <div class="page-nav" v-if="prev || next">
      <p class="inner">
        <span v-if="prev" class="prev">
          上一篇:
          <router-link :to="prev.path" :title="prev.frontmatter.title || ''">{{ prev.frontmatter.title || prev.frontmatter.description || prev.path }}</router-link>
        </span>
        <span v-if="next" class="next">
          下一篇:
          <router-link :to="next.path" :title="next.frontmatter.title || ''">{{ next.frontmatter.title || next.frontmatter.description || next.path }}</router-link>
        </span>
      </p>
    </div>
    <CommentBar v-if="!$page.frontmatter.noComment" :show="true" />
    <slot name="bottom"/>
  </main>
</template>
